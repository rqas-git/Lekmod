...
#else
	if (eReligionCreated > RELIGION_PANTHEON &&
		!HasAddedReformationBelief(ePlayer) &&
		(kPlayer.GetPlayerPolicies()->HasPolicyGrantingReformationBelief() || kPlayer.CanChooseReformationBelief()))
#endif
#endif
	{
		if (!kPlayer.isHuman())
	...
	case FAITH_PURCHASE_UNIT:
		{
			...
			if (!kPlayer.IsCanPurchaseAnyCity(false, false                                      , eUnit, NO_BUILDING, YIELD_FAITH))
			{
				bSelectionStillValid = false;
			}
			else
			{
				if (kPlayer.IsCanPurchaseAnyCity(true, true                          , eUnit, NO_BUILDING, YIELD_FAITH))
				{
	...
	case FAITH_PURCHASE_BUILDING:
		{
			...
			{
				if (kPlayer.IsCanPurchaseAnyCity(true, true                          , NO_UNIT, eBuilding, YIELD_FAITH))
				{
...
bool CvGameReligions::CheckSpawnGreatProphet(CvPlayer& kPlayer)
{
	...
	const int iFaith = kPlayer.GetFaith();
	int iCost = kPlayer.GetReligions()->GetCostNextProphet(true                            , true                              );
	...
	{
		pSpawnCity->GetCityCitizens()->DoSpawnGreatPerson(eUnit, false                    , true);
...
void CvCityReligions::DoPopulationChange(int iChange)
{
	...
	{
#if defined(LEKMOD_RELIGIOUS_PRESSURE_POP_LOSS)
		if (eMajorityReligion == NO_RELIGION)
		{
			AddReligiousPressure(FOLLOWER_CHANGE_POP_CHANGE, eMajorityReligion, iChange * GC.getRELIGION_ATHEISM_PRESSURE_PER_POP());
		}
...
void CvCityReligions::AddProphetSpread(ReligionTypes eReligion, int iPressure, PlayerTypes eResponsiblePlayer)
{
...
#endif
	{
		...
		if (it->m_eReligion > RELIGION_PANTHEON &&  it->m_eReligion != eReligion)
		{
			...
			int iPressureRetention = pReligion->m_Beliefs.GetInquisitorPressureRetention();
	...
	if (eHolyCityReligion != NO_RELIGION && !bProphetsReligionFoundedHere)
	{
		...
		{
			CvReligionInCity holyCityReligion(eHolyCityReligion, true                , 0, iPressureRetained);
			m_ReligionStatus.push_back(holyCityReligion);			
		}
		else
		{
			CvReligionInCity holyCityReligion(eHolyCityReligion, true                , 0, 0);
			m_ReligionStatus.push_back(holyCityReligion);
		}
	}
...
void CvCityReligions::AddReligiousPressure(CvReligiousFollowChangeReason eReason, ReligionTypes eReligion, int iPressure, PlayerTypes eResponsiblePlayer)
{
...
#endif
	{
		...
		{
			const CvReligion *pReligion = GC.getGame().GetGameReligions()->GetReligion(eReligion, NO_PLAYER);
			int iPressureErosion = pReligion->m_Beliefs.GetOtherReligionPressureErosion();
...
void CvCityReligions::RemoveOtherReligions(ReligionTypes eReligion, PlayerTypes eResponsiblePlayer)
{
...
#endif
	{
		...
		if (eLoopReligion > RELIGION_PANTHEON && eLoopReligion != eReligion)
		{
			...
			iPressureRetained = pReligion->m_Beliefs.GetInquisitorPressureRetention();
		}
...
void CvCityReligions::RecomputeFollowers(CvReligiousFollowChangeReason eReason, ReligionTypes eOldMajorityReligion, PlayerTypes eResponsibleParty)
{
...
#endif
	{
		...
		it->m_iTemp = it->m_iPressure - (it->m_iFollowers * iPressurePerFollower);
	}
		...
		if(eMajority != eOldMajority && eResponsibleParty != NO_PLAYER)
		{
			...
			if(pNewReligion->m_eFounder != m_pCity->getOwner())
			{
				...
				if(eCityOwnerReligion >= RELIGION_PANTHEON)
				{
					...
					{
						iPoints = GC.getRELIGION_DIPLO_HIT_INITIAL_CONVERT_FRIENDLY_CITY();
					}


					else if(m_pCity->GetCityReligions()->IsHolyCityForReligion(eCityOwnerReligion))
					{
						iPoints = GC.getRELIGION_DIPLO_HIT_CONVERT_HOLY_CITY();
					}


					else
					{
						iPoints = GC.getRELIGION_DIPLO_HIT_RELIGIOUS_FLIP_FRIENDLY_CITY();
					}
...
#endif
{
	...
	if(pCity->getOwner() == m_pPlayer->GetID())
	{
		...
		{
			iTurns = TurnsToReachTarget(pUnit, pCity->plot(), true                  );
			if(iTurns < MAX_INT)
			{
				*piTurns = iTurns;
				return pCity->plot();
...
#pragma warning ( pop )
	...
	for(int iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
	{
		...
		if(pLoopPlot != NULL)
		{
			...
			{
				iTurns = TurnsToReachTarget(pUnit, pLoopPlot, true                  );
				if(iTurns < MAX_INT)
				{
		...
		for(int iPlayerLoop = 0; iPlayerLoop < MAX_CIV_PLAYERS; iPlayerLoop++)
		{
			...
			if(kLoopPlayer.isAlive() && iPlayerLoop != m_pPlayer->GetID())
			{
				...
				if ((GC.getGame().isOption("GAMEOPTION_AI_TWEAKS") || GC.getGame().isOption("GAMEOPTION_AI_GIMP_NO_RELIGION_SPREAD")) && kLoopPlayer.isHuman())
					continue;
#endif
#ifdef NQ_AI_GIMP_NO_MINOR_SPREAD
...
#endif
{
	CvCity* pCity = ChooseProphetConversionCity(false                                    );
	int iBestNumTurns = MAX_INT;
	int iTurns;
	int iBestDistance = MAX_INT;
	int iDistance;
...
void CvReligionAI::DoFaithPurchases()
{
	...
	else
	{
		...
		if (eProphetType != NO_UNIT && ChooseProphetConversionCity(true                                    ) && m_pPlayer->GetReligions()->GetNumProphetsSpawned() <= 5)
		{
		...
		else if(!bTooManyMissionaries && !AreAllOurCitiesConverted(eReligion, false                    ))
		{
...
#ifdef AUI_RELIGION_FIX_MULTIPLE_FAITH_BUILDINGS
		else if (eFaithBuilding.size() > 0 && !AreAllOurCitiesHaveFaithBuilding(eReligion, false                    ))
		{
...
#else
		else if(eFaithBuilding != NO_BUILDINGCLASS && !AreAllOurCitiesHaveFaithBuilding(eReligion, false                    ))
		{
...
#ifdef AUI_RELIGION_FIX_MULTIPLE_FAITH_BUILDINGS
		else if (!bTooManyMissionaries && eFaithBuilding.size() > 0 && !AreAllOurCitiesHaveFaithBuilding(eReligion, true                    ))
#else
		else if(!bTooManyMissionaries && eFaithBuilding != NO_BUILDINGCLASS && !AreAllOurCitiesHaveFaithBuilding(eReligion, true                    ))
#endif
		{
			BuyMissionary(eReligion);
		...
		else if(!bTooManyMissionaries && HaveNearbyConversionTarget(eReligion, false                               ))
		{
			BuyMissionary(eReligion);
		...
		else if(!bTooManyMissionaries && HaveNearbyConversionTarget(eReligion, true                               ))
		{
			BuyMissionary(eReligion);
...
void CvReligionAI::BuyMissionary(ReligionTypes eReligion)
{
	...
	if (pCapital)
	{
		int iCost = pCapital->GetFaithPurchaseCost(eMissionary, true                            );
...
void CvReligionAI::BuyInquisitor(ReligionTypes eReligion)
{
	...
	if (pCapital)
	{
		int iCost = pCapital->GetFaithPurchaseCost(eInquisitor, true                            );
		if (iCost <= kPlayer.GetFaith())
		{
...
UnitTypes CvReligionAI::GetDesiredFaithGreatPerson() const
{
...
#endif
	{
		...
		if (pCapital)
		{
			if (pCapital->IsCanPurchase(false                     , false                  , eUnit, NO_BUILDING, NO_PROJECT, YIELD_FAITH))
			{
				int iScore = 0;
				...
				if (eUnitClass == GC.getInfoTypeForString("UNITCLASS_PROPHET"))
				{
					...
					{
						if (ChooseProphetConversionCity(true                                    ))
						{
							iScore = 1500;
						}
						else if (ChooseProphetConversionCity(false                                    ))
						{
							iScore = 750;
						}
				...
				{
					if (HaveNearbyConversionTarget(eReligion, false                               ))
					{
		...
		if (pkUnitEntry->IsSpreadReligion() || pkUnitEntry->IsRemoveHeresy())
		{
			...
			if (eReligion > RELIGION_PANTHEON)
			{
				for(pLoopCity = kPlayer.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kPlayer.nextCity(&iLoop))
				{
					...
					if (pLoopCity->GetCityReligions()->GetReligiousMajority() == eReligion)
					{
						if(pLoopCity->IsCanPurchase(false                      , true, eUnit, NO_BUILDING, NO_PROJECT, YIELD_FAITH))
						{
							return true;
						}
					}
